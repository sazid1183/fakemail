...
<?php 
    ...
    $sql_adan = "SELECT * FROM `{$session_email}` WHERE `action` = 'sent' ORDER BY `{$session_email}`.`mail_id` DESC LIMIT {$offset}, {$limit} ";
    $final_sql_adan = mysqli_query($connection, $sql_adan);
    $count = mysqli_num_rows($final_sql_adan);
    $sql_adan_unseen = "SELECT * FROM `{$session_email}` WHERE `action` = 'sent'";
    $final_sql_adan_unseen = mysqli_query($connection, $sql_adan_unseen);
    $count_unseen = mysqli_num_rows($final_sql_adan_unseen);

    
?>
    <div class="my-3 col-11 d-block mx-auto"><p class="h4 text-primary">Hi <?php echo $session_name;?>. You have been sended <?php echo $count_unseen; ?> message.</p></div>
     <div class="allmail">
...
<?php
    $query_pagination = "SELECT * FROM `{$session_email}` WHERE `action` = 'sent'";
...
<form action="sent" method="post" class="d-md-inline d-none">
...
<form action="sent" method="post" class="d-md-none d-inline">
